...
{   // {{{
    ...
    constexpr auto show_error_message = [](const toml::key& section, const toml::key& value) noexcept -> void
    // [Abstract]
    ...
    else if ((section == "COMPLETION") and (value == "completions"))
    {
        for (const auto& entry : *node.as_array())
        {
            ...
            constexpr auto get_value = [](const toml::node& token) noexcept -> std::string { return token.value_or(""); };
